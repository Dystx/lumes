...
import { splitClassName, parseStyleObject } from '../utils';
...
function toScaleUnits(numeric: number, unit: 'px' | 'rem'): number {
  // Tailwind scale: 1 unit == 0.25rem == 4px.
...
function checkClassToken(token: string, scale: readonly number[]): number | undefined {
  ...
  if (raw === 'px') {
    // Tailwind *-px == 1px == 0.25 scale units. The default scale does not
    // include 0.25, but this is a standard Tailwind half-step token.
    return undefined;
  }

  if (SKIP_VALUES.has(raw)) {
    return undefined;
  }
...
function checkInlineValue(value: string, scale: readonly number[]): number[] {
  ...
  while ((match = re.exec(value)) !== null) {
    ...
    if (!isInScale(scaleUnits, scale)) {
      offenders.push(scaleUnits);
    }
  }
  return offenders;
}
...
export const spacingGridRule = createRule<SpacingGridContext>({
  ...
  analyze(context: SpacingGridContext, facts: ScanFacts): Issue[] {
    ...
    for (const fact of facts.staticClassNames) {
    ...
    for (const fact of facts.styleProps) {
      for (const entry of parseStyleObject(fact.source)) {
