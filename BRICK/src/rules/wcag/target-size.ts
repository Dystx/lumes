...
function parseSimpleSelector(selector: string): ParsedSelector | undefined {
  ...
  // Optional type selector at the start (an identifier, not a special char).
  ...
  // Reject selectors with unsupported combinators or pseudo syntax.
  if (/[\s>+~:]/.test(rest)) return undefined;

  return result;
}

function matchesSelector(element: ElementFact, selector: string): boolean {
  // Bare class-name selectors are accepted for backward compatibility.
  ...
  const elementClasses = new Set(
    element.classNames.flatMap((fact) => splitClassName(fact.value)),
  );
...
export const targetSizeRule = createRule<TargetSizeContext>({
  ...
  analyze(context: TargetSizeContext, facts: ScanFacts): Issue[] {
    ...
    for (const element of facts.interactiveElements) {
      ...
      const hasSizing = classes.some((className) => isMinimumSizingToken(className));
